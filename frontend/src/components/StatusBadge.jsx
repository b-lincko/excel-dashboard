const COLORS = {
  ...
  "ON HOLD": "bg-orange-50 text-orange-700 dark:bg-orange-500/10 dark:text-orange-300",
  Open: "bg-teal-50 text-teal-800 dark:bg-teal-500/10 dark:text-teal-200",
  OPEN: "bg-teal-50 text-teal-800 dark:bg-teal-500/10 dark:text-teal-200",
...
function colorFor(value) {
  ...
  const hit = Object.keys(COLORS).find((k) => k.toLowerCase() === lower);
  return hit ? COLORS[hit] : "bg-slate-100 text-slate-700 dark:bg-white/10 dark:text-slate-200";
}

export default function StatusBadge({ value }) {
  if (!value) return <span className="text-slate-400">—</span>;
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${colorFor(value)}`}>
      {value}
    </span>
  );
}
