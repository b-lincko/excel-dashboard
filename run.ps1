...
Write-Host "============================================================"
Write-Host "  Linkco MR Dashboard - install and run"
Write-Host "============================================================"
...
if (-not $python) {
    Write-Host "ERROR: Python 3.11+ was not found on PATH."
    Write-Host "Install from https://www.python.org/downloads/ and tick Add Python to PATH."
    Read-Host "Press Enter to close"
    exit 1
}
if (-not (Get-Command npm -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: Node.js / npm was not found on PATH."
...
if (-not (Test-Path -LiteralPath $venvPy)) {
    ...
    if ($LASTEXITCODE) {
        Read-Host "venv failed. Press Enter"
        exit 1
    }
}
...
if ($LASTEXITCODE) {
    Write-Host "ERROR: pip install failed. Delete the .venv folder and try again."
    Read-Host "Press Enter to close"
    exit 1
}
...
if (-not (Test-Path -LiteralPath $nm)) {
    ...
    if ($LASTEXITCODE) {
        Pop-Location
        Read-Host "npm install failed. Press Enter"
        exit 1
    }
    Pop-Location
} else {
...
if (-not $up) {
    Write-Host "WARNING: API did not respond yet. Check the Linkco MR API window."
}

$uiDir = Join-Path $Root "frontend"
$uiArg = "title Linkco MR UI && cd /d `"$uiDir`" && echo Starting http://127.0.0.1:5173 && echo Keep this window OPEN. && npm run dev -- --host 0.0.0.0 --port 5173 && echo. && echo UI stopped. && pause"
Start-Process -FilePath "cmd.exe" -ArgumentList @("/k", $uiArg) -WorkingDirectory $uiDir
