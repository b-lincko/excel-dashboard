...
- **2026-09-13 (SMB auto-fallback: mount missing -> smbclient push)** User's runs kept ending PARTIAL_S (SMB FAILED "plain folder, not a mounted share") because /mnt/mr-backup was never actually CIFS-mounted on their server. Instead of only refusing, `run_dual_backup` now RESCUES the off-site copy: when `SMB_MODE=mount` and `os.path.ismount()` is false, it checks whether the smbclient transport is usable (binary present + SMB_SERVER + SMB_USERNAME/SMB_PASSWORD in env) and, if so, switches THAT RUN to a direct smbclient push to //server/share (`SMB_FALLBACK_SMBCLIENT` logged, `status.smb.fallback="smbclient"`, verified by re-download+hash, run = SUCCESS) while `mount_warning` + an amber `smb.reason` keep telling the operator to fix the mount (Settings shows reason amber on SUCCESS, red on FAILED; admin_backups exposes `smb_fallback`). If the fallback is impossible the old honest behavior stays (FAILED -> PARTIAL_SUCCESS) and the reason now lists the exact blockers ("smbclient not installed", "SMB_USERNAME/SMB_PASSWORD not set"). smbclient calls gained `-m SMB3` (the Linkco server needs explicit negotiation - same reason mounts use vers=3.0). So: set the SMB_* credentials even in mount mode - they are the rescue path. docs/backup.md section 4 + .env.example updated. USER ACTION for a clean SUCCESS: either really mount //192.168.100.5/mr.backup (sudo bash setup-commands.sh, check `mountpoint /mnt/mr-backup`), or `sudo apt install smbclient` + set SMB_SERVER=192.168.100.5 SMB_SHARE=mr.backup SMB_DOMAIN=LINKCO.COM SMB_USERNAME=mr.backup SMB_PASSWORD=... in .env (wizard writes them), or set SMB_MODE=smbclient permanently. Tests: fallback happy path (fake smbclient run against a tmp "file server", asserts remote artifact + SUCCESS + both log events + nothing written into the fake plain folder) + no-creds case asserts "fallback unavailable" reason. 9/9 dual-backup tests pass.

- **2026-09-13 (material request FORM table)** User asked for a copy-friendly table above Material notes with: S/N, part model number, material description, technical specification, unit model number/details, brand, required qty, UOM, remarks. REBUILT LineItemsCard as "Material request form" spreadsheet table (data-tour=wo-lines kept): 11 columns = the 9 requested (S/N = row number, non-input) + Supplier + Date needed (kept - vendor/Excel flows depend on lines[0].supplier); plain borderless cell inputs (focus ring), NATIVE <datalist> for supplier/item/brand/UOM completion (absolute dropdowns would clip in overflow-x table); Alt+Enter now REALLY adds a row (was advertised, never wired); "Copy table" = TSV via clipboard API, fallback opens paste panel pre-filled; "Paste rows" = textarea, tab-split parser, 9+-col rows assume leading S/N (offset), copied header rows skipped by phrase match, appends rows, supplier mirror for row 1 kept. BACKEND: mr_lines += part_model/tech_spec/unit_model/brand (CREATE TABLE + PRAGMA migration like needed_date; normalize_lines + replace_mr_lines carry them; list SELECT * flows automatically; Excel writer untouched - never invent columns; unknown keys still dropped). Tests test_mr_line_form.py 4 (normalize+drop, persist/list roundtrip, API create+fetch, legacy lines). MR create requires description+status (validation.py). 158 passed.
