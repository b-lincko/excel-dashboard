...
from ..security import GUEST_PAGES, VALID_ROLES, require_permission
from .auth import public_user

router = APIRouter(prefix="/api/users", tags=["users"])


def _permissions_json(role: str, extra: Optional[list[str]]) -> Optional[str]:
    if role != "guest":
        return "[]"
    pages = [p for p in (extra or []) if p in GUEST_PAGES]
...
def update_user(user_id: int, body: UserUpdate, user=Depends(require_permission("users"))):
    ...
    elif payload.get("role") and payload["role"] != "guest":
