...
def check_account_names(values: dict[str, str]) -> list[dict[str, Any]]:
    """Classic field mistake: the SHARE name typed as the ACCOUNT name."""
    out: list[dict[str, Any]] = []
    for user_key, share_key, dom_key, tag in (
        ("SMB_USERNAME", "SMB_SHARE", "SMB_DOMAIN", "backup"),
        ("FILES_SMB_USERNAME", "FILES_SMB_SHARE", "FILES_SMB_DOMAIN", "files"),
    ):
        user = values.get(user_key, "").strip()
        share = values.get(share_key, "").strip()
        if not user:
            continue
        if share and user.lower() == share.lower():
            out.append({
                "id": f"smb-name:{tag}",
            ...
            out.append({
                ...
                "label": f"SMB account ({tag})",
                "status": "warn",
                "detail": f"{dom_key} looks like a placeholder - use the NETBIOS domain (e.g. LINKCO) or ask IT",
            })
    return out
...
def run_all_checks(values: dict[str, str]) -> list[dict[str, Any]]:
    ...
    results.extend(check_account_names(values))
...
def render_commands(v: dict[str, str]) -> str:
    ...
    out = [
        "#!/bin/sh",
        "# Generated by the Linkco MR setup wizard.",
        "# Run as your NORMAL user (bash ./setup-commands.sh) - it elevates with sudo where needed.",
        "# noperm keeps a root-made mount usable by the app user; the server's share ACLs decide access.",
        "set -x",
        "",
    ]
...
def _mount_error_hint(returncode: int, stderr: str) -> str:
    """Human decode of common mount.cifs failures."""
    text = (stderr or "") + f" (exit {returncode})"
    if returncode == 13 or "Permission denied" in text:
        return (
            "the file server REJECTED the account (mount error 13). Check, in order: "
            "1) SMB_USERNAME is the service ACCOUNT name (e.g. svc_mr_backup) - NOT the share name "
            "(mr.backup is the share, there is no such login); "
            "2) the DOMAIN (try LINKCO vs LINKCO.COM - ask IT which); "
            "3) the password; 4) the account actually has Modify rights on the share. "
            "Fix the form, re-Validate, Save (rewrites the cred file), mount again."
        )
    if returncode == 2 or "No such file or directory" in text:
        return "share name not found on the server - check SMB_SHARE spelling/case"
    if "Connection refused" in text or "host down" in text or "unreachable" in text:
        return f"could not reach the server - check the IP/firewall (SMB 445/tcp)"
    if "cifs-utils" in text or "mount.cifs" in text:
        return "cifs-utils missing: sudo apt install cifs-utils"
...
def try_mount(which: str, v: dict[str, str]) -> dict[str, Any]:
    ...
    return {"id": f"mount:{label}", "label": f"Mount {label} share", "status": "error", "detail": _mount_error_hint(proc.returncode, proc.stderr or "")}
