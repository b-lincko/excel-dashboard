...
def check_mounted(path_raw: str, label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "label": label,
        "status": "manual",
        "detail": f"{path} is a plain local folder (not a mount). Use 'Mount now' or run the commands from setup-commands.sh",
    }
...
def save_env(v: dict[str, str]) -> Path:
    ...
    cmd = render_commands(v)
    COMMANDS_PATH.write_text(cmd, encoding="utf-8")
    os.chmod(COMMANDS_PATH, stat.S_IRWXU)
    return ENV_PATH
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="row"><button class="b-mount" type="button" onclick="doMount('backup')">Mount backup share now</button>
      <span class="hint">Needs root (run the wizard with sudo). In smbclient mode this is not needed.</span></div>
  </div>
